...
const DharmicLoader = ({ visible }) => {
  ...
  React.useEffect(() => {
    if (!visible) return;
    const iv = setInterval(() => setMsgIndex((p) => (p + 1) % messages.length), 1600);
  ...
  return (
    <div
      ...
      style={{
        background: "linear-gradient(135deg,rgba(255,237,213,0.93),rgba(254,215,170,0.89),rgba(252,165,165,0.86))",
...
function UpdateAddress() {
  ...
  useEffect(() => {
    ...
    const fetchAddress = async () => {
      try {
        ...
        const address = res.data.user.addresses.find((a) => a._id === id);
  ...
  if (fetching) return <DharmicLoader visible />;
  ...
  return (
    <>
      ...
      <div
        ...
        style={{ background: "linear-gradient(160deg,#fff7ed 0%,#fef3c7 40%,#fee2e2 100%)" }}
      >
        ...
        <div className="w-full max-w-3xl mx-auto">
          ...
          >
            ...
            <div>
              ...
              <h1 className="text-2xl font-bold text-gray-900">Update Address</h1>
        ...
        <div className="h-1 w-full max-w-3xl mx-auto rounded-full bg-gradient-to-r from-orange-400 via-red-400 to-orange-500 opacity-40" />
      </div>
    </>
  );
}
